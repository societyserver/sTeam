...
int main(int argc, array(string) argv)
{
  ...
  object dest = find_object(options->dest);
  if (!(dest->get_object_class() & (CLASS_CONTAINER|CLASS_ROOM)) && sizeof(options->files) > 1)
  {
    werror("destination %O is not a container\n", dest);
    return(1);
  }
  ...
  return upload(options->file, options->dest, file);
}

object find_object(string filename)
{
    object file;
    if ((string)(int)filename == options->file)
      file = conn->find_object(filename);
    else if (filename[0] == '/')
      file = _Server->get_module("filepath:tree")->path_to_object(filename);
    else // FIXME: try to find out how to use relative paths
      file = _Server->get_module("filepath:tree")->path_to_object(filename);
    if (file->get_class() == "Link")
        file = file->get_link_object();
    if (file->get_object_class() & CLASS_EXIT)
        file = file->get_exit();
    return file;
}
...
mapping init(array argv)
{
  ...
  options->dest = argv[-1];
  options->files = argv[1..sizeof(argv)-2];
